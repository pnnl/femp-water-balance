...
const calculateUrinals = (occupancy, basePath, values, subgroup) => {
  ...
  if (selectn(`${basePath}.urinals`)(values) == 'Yes') {
    let flushRatePerHour = subgroup == 'inPatient' ? 4 : 0.25;
    if (subgroup == 'admin' || subgroup == 'staff') {
...
class PlumbingForm extends React.Component {
  ...
  calculateWaterUse = (values, valid) => {
    ...
    let totalPlumbingHospitalAdmin =
      hospitalAdminUrinals + hospitalAdminToilets + hospitalAdminRestroomSink + hospitalAdminKitchenSink + hospitalAdminShowers;
    let totalPlumbingHospitalStaffShowers =
      hospitalStaffUrinals + hospitalStaffToilets + hospitalStaffRestroomSink + hospitalStaffKitchenSink + hospitalStaffShowers;
    let totalPlumbingHospitalOutPatient =
      hospitalOutPatientUrinals +
      hospitalOutPatientToilets +
      hospitalOutPatientRestroomSink +
      hospitalOutPatientKitchenSink +
      hospitalOutPatientShowers;
    let totalPlumbinghospitalInPatient =
      hospitalInPatientUrinals + hospitalInPatientToilets + hospitalInPatientRestroomSink + hospitalInPatientKitchenSink + hospitalInPatientShowers;
    let totalPlumbingWeek =
      weekDaygeneralCampusUrinals +
      weekDaygeneralCampusToilets +
      weekDaygeneralCampusRestroomSink +
      weekDaygeneralCampusKitchenSink +
      weekDaygeneralCampusShowers;
    let totalPlumbingWeekend =
      weekendDaygeneralCampusUrinals +
      weekendDaygeneralCampusToilets +
      weekendDaygeneralCampusRestroomSink +
      weekendDaygeneralCampusKitchenSink +
      weekendDaygeneralCampusShowers;

    let totalWaterUse =
      totalPlumbingLodging +
      totalPlumbingHospitalAdmin +
      totalPlumbingHospitalStaffShowers +
      totalPlumbingHospitalOutPatient +
      totalPlumbinghospitalInPatient +
      totalPlumbingWeek +
      totalPlumbingWeekend;
    let totalHospital =
      totalPlumbingHospitalAdmin + totalPlumbingHospitalStaffShowers + totalPlumbingHospitalOutPatient + totalPlumbinghospitalInPatient;
  ...
  flushRate = (basePath, values, source, title, people) => {
    ...
    return (
      <Fragment>
        <Typography variant='subtitle1' style={subHeader} gutterBottom>
          {title}
        </Typography>
        <ExpansionPanel style={expansionPanel} expanded={selectn(`${basePath}.typical_flush_rate`)(values) !== undefined}>
            ...
            <Grid container alignItems='flex-start' spacing={16}>
              ...
              <Grid item xs={12}>
                <Field
                  ...
                  mask={DEFAULT_DECIMAL_MASK}
                  label={
                    'What is the typical flow rate of kitchenette faucet aerators in ' + source + '? Please put 0 if no kitchenettes are present.'
                  }
                  endAdornment={<InputAdornment position='end'>gpm</InputAdornment>}
                ></Field>
              </Grid>
              ...
              {source === 'overall campus' && flowRate == 0 && this.clearValues(['shower_usage'], basePath, values)}
              {source === 'hospital/medical clinic' && flowRate != 0 && flowRate != undefined && (
                <Fragment>
                  <Grid item xs={12}>
                    <Field
                      formControlProps={{fullWidth: true}}
              ...
              {source === 'hospital/medical clinic' &&
                flowRate == 0 &&
    ...
    return (
      <ExpansionPanel style={expansionPanel} expanded={selectn(`${basePath}.total_population`)(values) !== undefined}>
        ...
        <ExpansionPanelDetails>
          ...
          {selectn(`${basePath}.total_population`)(values) == 0 && this.clearValues(['operating_weeks', 'shift_weekday'], basePath, values)}
          {selectn(`${basePath}.total_population_weekends`)(values) == 0 &&
    ...
    return (
      <Fragment>
        <Grid item xs={12}>
          ...
          <ExpansionPanel style={expansionPanel} expanded={selectn(`plumbing.has_onsite_lodging`)(values) === true}>
            <ExpansionPanelSummary>
              <Field
