...
const calculator = createDecorator(
  {
    ...
    updates: {
      ws_ww_total: (minimumValue, allValues) =>
        numberFormat.format(
          (allValues.water_supply.ww || []).reduce((sum, value) => {
  ...
  },
  {
    ...
    updates: {
      ws_wu_total: (minimumValue, allValues) =>
        numberFormat.format(
          (allValues.water_supply.wu || []).reduce((sum, value) => {
            const n = toNumber(value);
            return sum + (isNaN(n) ? 0 : n);
          }, 0)
        ),
      ...
      return (
        <Fragment>
          {DateUtils.getMonthArray(new Date()).map((monthDate) => {
            const rowDiff =
              toNumber(selectn(`${basePath}.wu[${DateUtils.getMonth(monthDate)}]`)(values)) -
              toNumber(selectn(`${basePath}.ww[${DateUtils.getMonth(monthDate)}]`)(values));
              ...
              <Fragment key={DateUtils.getMonth(monthDate)}>
                ...
                {values.monthly_discharge && values.monthly_usage && (
                  <Grid item xs={12} sm={4}>
                    <MaterialInput
                      disabled
                      meta={{visited: true, error: hasWarning}}
          ...
          {values.monthly_usage && (
            <Grid item xs={12} sm={4}>
              <Field
                ...
                meta={{
                  visited: true,
                  error: overallUsageOK
                    ? null
                    : 'The sum of monthly water use does not match annual total water use entered above. Please double check your inputs.',
                }}
                endAdornment={<InputAdornment position='end'>kgal</InputAdornment>}
              />
            </Grid>
                ...
                <Fragment>
                  <Grid item xs={12}>
                    <FormControlLabel
                      ...
                      control={<Field name='monthly_usage' component={Checkbox} indeterminate={values.monthly_usage === undefined} type='checkbox' />}
                    />
                  </Grid>
                  <Grid item xs={12}>
                    <FormControlLabel
                      ...
                      control={
                        <Field name='monthly_discharge' component={Checkbox} indeterminate={values.monthly_discharge === undefined} type='checkbox' />
                      }
                    />
                  </Grid>
