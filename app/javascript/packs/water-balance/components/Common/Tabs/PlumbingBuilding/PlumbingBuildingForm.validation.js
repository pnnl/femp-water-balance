...
const validateFlushRates = (values, primaryBuildingType) => {
  const errors = {};
  if (!isWithinNumericRange(values.typical_flush_rate, 1.0, 5.0, true)) {
    errors['typical_flush_rate'] = 'The flush rate of toilets must be between 1.0 and 5.0 gpf.';
  }
  if (values.urinals == 'yes' && values.urinals != undefined) {
    if (!isWithinNumericRange(values.urinal_flush_rate, 0, 5.0 , true)) {
      errors['urinal_flush_rate'] = 'The flush rate of urinals must be between 0 and 5.0 gpf.';
    }
  }
  if (!isWithinNumericRange(values.aerator_flow_rate, 0.25, 5.0, true)) {
    errors['aerator_flow_rate'] = 'The flow rate of restroom faucet aerators must be between 0.25 and 5.0 gpm.';
  }
  if (!isWithinNumericRange(values.kitchenette_flow_rate, 0, 5.0, true)) {
    if (values.kitchenette_flow_rate != 0) {
      errors['kitchenette_flow_rate'] =
        'If present the flow rate of kitchenette faucet aerators must be between 0 and 5.0 gpm.';
    }
  }
  if (!isWithinNumericRange(values.shower_flow_rate, 0, 5.0, true)) {
    if (values.shower_flow_rate != 0) {
      errors['shower_flow_rate'] = 'If present the flow rate of showers must be between 0 and 5.0 gpm.';
    }
  }
