...
const validateNoSoftner = (values) => {
    ...
    if (!isWithinNumericRange(valuePath, 1, 168)) {
        errors['hours_week'] = 'The number of hours the system operates per week between 1 and 168.';
    }
    return Object.keys(errors).length === 0 ? undefined : errors;
}
    ...
    if (values.is_metered == 'yes') {
        ...
        if (valuePath != undefined) {
            ...
            if (!isWithinNumericRange(valuePath, 1, 52)) {
                errors['operating_weeks'] = 'The number of weeks per year the system is operating must be between 1 and 52.';
            }
        }
    } 
